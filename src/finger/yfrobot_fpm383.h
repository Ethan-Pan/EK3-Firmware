...
class YFROBOTFPM383
{
  private:
    ...
    uint8_t PS_AutoEnrollBuffer[17] = { 0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00, 0x08, 0x31, '\0', '\0', '\0', 0x00, 0x2F, '\0', '\0' };
    // 自动注册，
    // 录入次数：1~12，推荐值4~6
    // 参数说明：最低位为 bit0。
    ...
    // 当前值为 0x17，0001 0111；灯光获取成功后熄灭，打开预处理，不要求返回状态，不允许覆盖ID，不允许重复注册，多次采集手指需要离开
                    // 0x 2F 0010 1111 灯光获取成功后熄灭，打开预处理，不要求返回状态，允许覆盖ID，允许重复注册，多次采集手指不需要离开
